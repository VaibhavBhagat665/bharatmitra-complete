...
const LeaderboardPage: React.FC = () => {
      ...
      (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
          ...
          users.push({
            ...
            occupation: data.occupation || 'Government Scheme Explorer',
            badge: data.badge || getBadgeFromTokens(data.bharat_tokens || 0),
  ...
  // Helper function to determine badge based on tokens
  const getBadgeFromTokens = (tokens: number): string => {
    if (tokens >= 2000) return 'Champion Master';
    if (tokens >= 1500) return 'Expert Champion';
    if (tokens >= 1000) return 'Scholar Expert';
    if (tokens >= 500) return 'Learner Scholar';
    if (tokens >= 100) return 'Explorer Learner';
    return 'New Explorer';
  };

  const getUserProgress = () => {
    const level = Math.floor((userData?.bharat_tokens || 0) / 100) + 1;
    const tokensToNextLevel = 100 - ((userData?.bharat_tokens || 0) % 100);
    return { level, tokensToNextLevel };
  };

  const getUserRank = () => {
    if (!user || !userData) return null;
    const userIndex = leaderboardData.findIndex(u => u.uid === user.uid);
    return userIndex !== -1 ? userIndex + 1 : null;
  };

  const getUserBadge = () => {
    return userData?.badge || getBadgeFromTokens(userData?.bharat_tokens || 0);
  };
  ...
  const getBadgeColor = (badge: string) => {
    if (badge.includes('Champion') || badge.includes('Master')) return 'bg-gradient-to-r from-yellow-400 to-orange-500';
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          ...
          <div className="flex gap-2">
            ...
            {['all', 'students', 'professionals'].map(filter => (
              <button
                key={filter}
                      ...
                      <div className="col-span-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <span className="text-lg font-bold text-gray-900">
                            {leaderUser.bharat_tokens.toLocaleString()}
                          </span>
                        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {leaderboardData.reduce((sum, user) => sum + user.bharat_tokens, 0).toLocaleString()}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {leaderboardData.filter(user => user.bharat_tokens >= 1000).length}
                </p>
              </div>
            </div>
          </div>
